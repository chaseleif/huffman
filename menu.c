...
static void compressionscreen();

// active display for decompression
static void decompressionscreen();
...
enum fancycolors { HFCBLKGRN=1,HFCBLKCYN=2,HFCWHTBLK=4,HFCWHTBLU=5,HFCCYNBLU=6,HFCCYNBLK=7,HFCGRNBLK=8,HFCREDBLK=9,HFCREDCYN=10,HFCBLUWHT=11 };
...
static int setmenuoptions(const unsigned char menulevel) {
	...
	while (1) {
		...
		if (menulevel==MAINMENU) {
			...
			else if (i==3) strcpy(menuoptions[i],"Free compression data structures\n");
...
static inline void drawopenerrorandgetch(const int row,const int col,char *filename) {
	const char *const errormsg = "Press the any key to continue . . .";
	mvwprintw(stdscr,row,col,"Unable to open \"%s\" !",filename);
	wattron(stdscr,A_STANDOUT);
	mvwprintw(stdscr,row+2,col,errormsg);
	wattroff(stdscr,A_STANDOUT);
	wmove(stdscr,row+2,col+strlen(errormsg));
	curs_set(2);
	wgetch(stdscr);
	curs_set(0);
}
...
// compress screen
void compressionscreen() {
	wclear(stdscr);
	wrefresh(stdscr);
	mvwprintw(stdscr,TITLELINENUM,CENTERMARGIN,"%s",titlebar);
/*
	...
	fclose(infile);
	mvwprintw(stdscr,TITLELINENUM+2,LEFTMARGIN,"Running compression, input = \"%s\" (%llu bytes), output = \"%s\", byte grouping size = %u, number passes = %u",compressinfile,insize,compressoutfile,NUMTHREADS,NUMPASSES);
...
// decompress screen
void decompressionscreen() {
	wclear(stdscr);
	wrefresh(stdscr);
	mvwprintw(stdscr,TITLELINENUM,CENTERMARGIN,"%s",titlebar);
	mvwprintw(stdscr,TITLELINENUM+2,LEFTMARGIN,"Running decompression, input = \"%s\", output = \"%s\"",decompressinfile,decompressoutfile);
//	const int topline = TITLELINENUM+4;
	box(stdscr,0,0);
//	wborder(stdscr,'|','|','_','_',' ',' ','|','|');
	wrefresh(stdscr);
...
//	w = newwin(0,0,0,0); //new window, (height,width,starty,startx)
	...
	while (1) {
		...
		wattroff(stdscr,A_STANDOUT);
		const int leftstop = CENTERMARGIN;
		wattr_on(stdscr,COLOR_PAIR(HFCGRNBLK),NULL);
		mvwprintw(stdscr,TITLELINENUM,leftstop,"%s",titlebar);
		wattr_off(stdscr,COLOR_PAIR(HFCGRNBLK),NULL);
		while (menuoptions[i][0]) {
			if (i==highlight)
				wattron(stdscr,A_STANDOUT); //active highlighted option
			else {
				wattroff(stdscr,A_STANDOUT);
				if (menuoptions[i][0]=='F' && menuoptions[i][1]=='r' && menuoptions[i][2]=='e' && !hfcroot)
					wattron(stdscr,A_INVIS); // A_DIM (dim)
			}
			mvwprintw(stdscr,i+MENUFIRSTLINE,leftstop,"%s",menuoptions[i]);
			wattroff(stdscr,A_INVIS);
			++i;
		}
//		wborder(stdscr,'|','|','_','_',' ',' ','|','|');
		wattr_on(stdscr,COLOR_PAIR(HFCBLKGRN),NULL);
		box(stdscr,0,0);
		wrefresh(stdscr);
		wattr_off(stdscr,COLOR_PAIR(HFCBLKGRN),NULL);
